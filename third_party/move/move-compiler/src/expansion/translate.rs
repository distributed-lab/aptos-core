...
fn exp_(context: &mut Context, sp!(loc, pe_): P::Exp) -> E::Exp {
    ...
    let e_ = match pe_ {
        ...
        PE::Lambda(pbs, pe, capture_kind, abilities_vec, spec_opt) => {
            ...
            match tbs_opt {
                Some(tbs) => EE::Lambda(tbs, Box::new(e), capture_kind, abilities, ee_spec),
                None => {
                    assert!(context.env.has_errors());
                    EE::UnresolvedError
                },
...
fn unbound_names_exp(unbound: &mut UnboundNames, sp!(_, e_): &E::Exp) {
    ...
    match e_ {
        ...
        EE::Lambda(ls, er, _capture_kind, _abilities, spec_opt) => {
